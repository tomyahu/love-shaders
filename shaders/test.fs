...
extern float drop_expansion_speed;
...
vec4 effect( vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords ){
    ...
    for (i = 0; i < 100; i++){
        ...
        float final_color_norm = 1 - abs(normalized_distance_from_drop - drop_expansion_speed * (time - times[i]))/drop_width;
        final_color_norm =  final_color_norm*max(0, (time_to_disappear - (time - times[i])));

        final_color = max(final_color, final_color_norm);
    }

    vec4 color1 = vec4(final_color, 0.0, final_color, 1.0);
    vec4 color2 = vec4(final_color, 0.0, final_color, 1.0);

    vec4 texturecolor = Texel(tex, texture_coords);

    return max(texturecolor, color1 * color2);
}