...
vec4 effect( vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords ){
    ...
    p2 = vec2(p4.y, p4.x);
    p4 = vec2(p3.y, p3.x);
    p3 = vec2(aux_p1.y, aux_p1.x);

    b1 = - p.y*(p1.x - p2.x - p3.x + p4.x);
    b2 =   p.x*(p1.y - p2.y);
    b3 = - (p3.x*p2.y + p1.y*p4.x - 2*p4.x*p2.y);
    b4 = p2.x*p3.y - p.x*p3.y;
    b5 = p1.x*p4.y - 2*p2.x*p4.y + p.x*p4.y;

    c1 = - p.y*(p2.x - p4.x);
    c2 = p.x*p2.y;
    c3 = - (p4.x*p2.y);
    c4 = 0;
    c5 = p2.x*p4.y - p.x*p4.y;
